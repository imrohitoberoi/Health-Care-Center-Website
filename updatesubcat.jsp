...
<% 
    String subcname="", subcpic="",cati="",qual=""; 
    try              
     {    //TAKING INFO FROM MANAGESUBCAT TO BE USED DOWN WHILE UPDATING
            Class.forName("com.mysql.jdbc.Driver");
            Connection myconn =  DriverManager.getConnection(dburl, dbuname, dbpass);
            try
            {
                String subcatid1=request.getParameter("scid");
                String q = "select * from managesubcat where subcatid=?";//CATID IS AUTO INCREMENTED IN OUR TABLE
                PreparedStatement myst = myconn.prepareStatement(q);
                myst.setString(1, request.getParameter("scid"));//SCID WE CALLED FROM LAST PAGE(managesubcat) TO GET ITS ALL INFO(OF SUBCAT)
...
<section class="mail pt-lg-5 pt-4">
 <div class="container pt-lg-5">
  <h2 class="heading text-center mb-sm-5 mb-4">Update Doctor </h2>
                    ...
                    if(myres.next()==true)//HERE IT CHECKS IF THERE IS NEXT LINE OR NOT,,EVEN IF THERE IS ONE LINE(IT IS IN CASE PRIMARY KEYWORD, SINCE IT IS UNIQUE e.g. username here, as there is unique username for everyone so there will be one line only)
                       //IT HAS INITIAL CURSOR BEFORE THE FIRST LINE                    
                     {
                            ...
                            {    //IMP- HERE NOW CURSOR IS AT FIRST LINE/ROW OF OUR DATABASE AND WE CAN CALL ANY COLUMN USNG myres.getString("column name");
                                ...
                                }//HERE CAT WILL BE ALREADY SELECTED IN COMBOBOX
                                else
                                {
	...
	<div class="form-group contact-forms">
            <input type="text" name="scatname" value="<%out.print(subcname);%>" class="form-control" placeholder="Doctor name">
        </div>
        <div class="form-group contact-forms">
            <input type="text" name="qual" value="<%out.print(qual);%>" class="form-control" placeholder="Doctor name">
        </div>
        <%out.print("<img src='uploads/"+ subcpic + "' height='75'>");%><br/>
         Choose new image, if required<br/>
                <div class="form-group">
	<input type="file" name="subcpic" class="form-control">	
        </div>
	<div class="col-md-12">
  ...
  <%        // THIS IS FILE UPLOADING 
         String scname="", imagename="",path="",cid="",qual1="";//TWO STRINGS ARE DECLARED 
   ...
   for(FileItem myitem:items)//FOR EACH LOOP
   {
        if (myitem.isFormField())//EVERY ITEM EXCEPT FILE IS FORMFIELD
        {
            ...
            else if(itemName1.equals("scatname"))
            {
                scname=value;
            }
             else if(itemName1.equals("qual"))
            {
                qual1=value;
            }
        }
   ...
   if(successful==true)
   {
        try              
        {
            ...
            try
            {
                String q = "update managesubcat set catid=?, subcatname=?,qual=?, subcatpic=? where subcatid=?";//CATID IS AUTO INCREMENTED IN OUR TABLE
                PreparedStatement myst = myconn.prepareStatement(q);
                myst.setString(1, cid);
                myst.setString(2, scname);
                myst.setString(3, qual1);
                myst.setString(4, imagename);
                myst.setString(5, request.getParameter("scid"));//SINCE SCID WILL BE UPDATED ONLY
                int myres = myst.executeUpdate();
                if(myres>0)
                {
                    response.sendRedirect("managesubcat.jsp");
                                        out.print("Doctor updated successfully");
                    //response.sendRedirect("managesubcat.jsp");
                }
                else
                {
                    out.print("Doctor not updated successfully");
                }
            }
            catch(Exception e)
            {
