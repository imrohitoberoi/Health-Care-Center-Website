...
<h2 class="heading text-center mb-sm-5 mb-4">List of Staff</h2>
           ...
           try
             {
              ...
              try
               {
                    String q="select * from staff";//THIS STATEMENT IS VERY IMP SINCE IT SELECTS EVERYTHING(*) FROM SIGN UP(NAME OF OUR DATABASE)
                       ...
                       //IT HAS INITIAL CURSOR BEFORE THE FIRST LINE                    
                     {
                         out.print("<table class='table table-striped'>"   //HERE WE DIRECTLY PRINT THE LISTOFMEMBERS
                            ...
                            {    //IMP- HERE NOW CURSOR IS AT FIRST LINE/ROW OF OUR DATABASE AND WE CAN CALL ANY COLUMN USNG myres.getString("column name");
                                out.print("</tr>"
                                   + "<th>" + myres.getString("name") + "</th>"//myres.getString("column name")- GIVES THE VALUE OF THAT COLUMN AFTER WE ARE CONNECTED TO DATABASE
                                   + "<th>" + myres.getString("phone") + "</th>"
                                   + "<th>" + myres.getString("email") + "</th>"
                                   + "<th>" + myres.getString("gender") + "</th>"
                                   + "<th>" + myres.getString("qualification") + "</th>"
                                   + "<th>" + myres.getString("address") + "</th>"
                                   + "<th>" + myres.getString("type") + "</th>"
                                   + "<td><a href='updatestaff.jsp?sno=" + myres.getString("Sno") + "' style='color:black'>Update</a></td>"
                                   + "<td><a href='deletestaff.jsp?sno=" + myres.getString("Sno") + "' style='color:black'>Delete</a></td>");
