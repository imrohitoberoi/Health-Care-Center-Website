...
<h2 class="heading text-center mb-sm-5 mb-4">List of Appointments</h2>
           ...
           try
             {
              ...
              try
               {
                    String q="select * from appointment";//THIS STATEMENT IS VERY IMP SINCE IT SELECTS EVERYTHING(*) FROM SIGN UP(NAME OF OUR DATABASE)
                    ...
                    if(myres.next()==true)//HERE IT CHECKS IF THERE IS NEXT LINE OR NOT,,EVEN IF THERE IS ONE LINE(IT IS IN CASE PRIMARY KEYWORD, SINCE IT IS UNIQUE e.g. username here, as there is unique username for everyone so there will be one line only)
                       //IT HAS INITIAL CURSOR BEFORE THE FIRST LINE                    
                     {
                         out.print("<table class='table table-striped'>"   //HERE WE DIRECTLY PRINT THE LISTOFMEMBERS
                                 ...
                                 + "<th>Phone</th>"
                                 + "<th>Date</th>"
                                 + "<th>Delete</th></tr>");//WE WRITE HTML CODE IN - ""
                            ...
                            {    //IMP- HERE NOW CURSOR IS AT FIRST LINE/ROW OF OUR DATABASE AND WE CAN CALL ANY COLUMN USNG myres.getString("column name");
                                out.print("</tr>"
                                        + "<th>" + myres.getString("ano") + "</th>"//myres.getString("column name")- GIVES THE VALUE OF THAT COLUMN AFTER WE ARE CONNECTED TO DATABASE
                                                + "<th>" + myres.getString("SubcatID") + "</th>"
                                                + "<th>" + myres.getString("sno") + "</th>"
                                                + "<th>" + myres.getString("name") + "</th>"
                                                + "<th>" + myres.getString("phone") + "</th>"
                                                + "<th>" + myres.getString("date") + "</th>"
                                               + "<td><a href='deleteapp.jsp?ano=" + myres.getString("ano") + "' style='color:black'>Delete</a></td>");
                            }
                            while(myres.next());
