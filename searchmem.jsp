...
<h2 class="heading text-center mb-sm-5 mb-4">Search</h2>
...
<div class="col-lg-8 agileinfo_mail_grid_right">
	...
	<div class="form-group">
		<input type="text" name="uname" class="form-control" placeholder="Username/Phone/Email id" required="">
		</div>
		</div>
	
	<div class="col-md-12">
	<div class="submit-buttons">
            <input type="submit" name="s1" class="btn" value="Submit"> <br/><br/><br/>
		</div>
		</div>
		</div>
    <%
              ...
              try
               {
                ...
                String q="select * from signup where username=? or phonenumber=? or email=?";//THIS STATEMENT IS VERY IMP SINCE IT SELECTS EVERYTHING(*) FROM SIGN UP(NAME OF OUR DATABASE)
                ...
                if(myres.next()==true)//HERE IT CHECKS IF THERE IS NEXT LINE OR NOT,,EVEN IF THERE IS ONE LINE(IT IS IN CASE PRIMARY KEYWORD, SINCE IT IS UNIQUE e.g. username here, as there is unique username for everyone so there will be one line only)
                   //IT HAS INITIAL CURSOR BEFORE THE FIRST LINE                    
                 {
                    //IF SEARCH IS SUCCESSFUL WE SHOW THEIR INFORMATION USING myres.getString("column name") FUNCTION IN TABULAR FORM
                        ...
                        out.print("<tr> <th>Phone No.<th/>" + "<td>" + myres.getString("phonenumber") + "<td/>" + "<tr/>");
                        out.print("<tr> <th>Email<th/>" + "<td>" + myres.getString("email") + "<td/>" + "<tr/>");
                        out.print("<tr> <th>Usertype<th/>" + "<td>" + myres.getString("USERTYPE") + "<td/>" + "<tr/></table>");
                 }
                else
                 {
